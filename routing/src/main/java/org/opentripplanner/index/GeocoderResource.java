...
public class GeocoderResource {
     ...
     * @param rooms Search from rooms in buildings
    ...
    public Response textSearch (@QueryParam("query") String query,
                                ...
                                @QueryParam("rooms") @DefaultValue("true") boolean rooms
                                ) {
        return Response.status(Response.Status.OK).entity(index.query(query, autocomplete, stops, clusters, corners, rooms)).build();
    }
