...
public class CandidateEdge {
     ...
     */
    protected Vertex endwiseVertex;
     ...
     */
    public CandidateEdge(Edge e, GenericLocation loc, double pref, TraverseModeSet mode) {
        preference = pref;
        edge = e;
    ...
    public CandidateEdge(Edge e, Coordinate p, double pref, TraverseModeSet mode) {
    ...
    private Vertex calcEndwiseVertex() {
        Vertex retV = null;
        if (nearestSegmentIndex == 0 && Math.abs(nearestSegmentFraction) < 0.000001) {
            if (edge instanceof StreetEdge) retV = (StreetVertex) edge.getFromVertex();
            else if (edge instanceof IndoorEdge) retV = (IndoorVertex) edge.getFromVertex();
        } else if (nearestSegmentIndex == numEdgeCoords - 2
                && Math.abs(nearestSegmentFraction - 1.0) < 0.000001) {
        	if (edge instanceof StreetEdge) retV = (StreetVertex) edge.getToVertex();
        	else if (edge instanceof IndoorEdge) retV = (IndoorVertex) edge.getToVertex();
        }
        return retV;
    }
    ...
    private double calcScore(TraverseModeSet modes) {
        ...
        if (edge instanceof StreetEdge){
	        if ((((StreetEdge)edge).getStreetClass() & platform) != 0) {
	        ...
	        if (modes.getCar() && ((StreetEdge)edge).getPermission().allows(StreetTraversalPermission.CAR)
	                || (((StreetEdge)edge).getStreetClass() & platform) != 0) {
