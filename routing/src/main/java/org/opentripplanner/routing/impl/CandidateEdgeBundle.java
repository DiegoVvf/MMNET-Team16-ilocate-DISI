...
public class CandidateEdgeBundle extends ArrayList<CandidateEdge> {
    ...
    public List<StreetEdge> toStreetEdgeList() {
        List<StreetEdge> ret = new ArrayList<StreetEdge>();
        for (CandidateEdge ce : this) {
            if(ce.edge instanceof StreetEdge) ret.add((StreetEdge)ce.edge);
        }
        return ret;        
    }
    ...
    public boolean isPlatform() {
        for (CandidateEdge ce : CandidateEdgeBundle.this) {
            ...
            if(e instanceof StreetEdge){
	            if ((((StreetEdge)e).getStreetClass() & StreetEdge.ANY_PLATFORM_MASK) != 0) {
	                return true;
	            }
            }
        }
    ...
    public boolean allowsCars() {
        for (CandidateEdge ce : CandidateEdgeBundle.this) {
            Edge e = ce.edge;
            if(e instanceof StreetEdge){
	            if (((StreetEdge)e).getPermission().allows(StreetTraversalPermission.CAR)) {
	                return true;
	            }
            }
        }
