...
public abstract class GraphPathToTripPlanConverter {
    ...
    public static TripPlan generatePlan(List<GraphPath> paths, RoutingRequest request) {
        ...
        Place from = new Place(tripStartVertex.getX(), tripStartVertex.getY(), request.from.getLevel(), startName);
        Place to = new Place(tripEndVertex.getX(), tripEndVertex.getY(), request.to.getLevel(), endName);
    ...
    private static Place makePlace(State state, Vertex vertex, Edge edge, Stop stop, TripTimes tripTimes) {
        ...
        Place place = new Place(vertex.getX(), vertex.getY(), getLevel(vertex), vertex.getName(),
    ...
    public static List<WalkStep> generateWalkSteps(Graph graph, State[] states, WalkStep previous) {
        ...
        for (int i = 0; i < states.length - 1; i++) {
            ...
            if (edge instanceof IndoorStairsEdge) {
            	step = createWalkStep(graph, forwardState);
            	createdNewStep = true;
            	step.streetName = (((IndoorStairsEdge)edge).getName() != null) ? ((IndoorStairsEdge)edge).getName() : "Stairs";
            	
            	if(((IndoorVertex)edge.getFromVertex()).getLevel() > ((IndoorVertex)edge.getToVertex()).getLevel())
            		step.relativeDirection = RelativeDirection.GO_DOWN_THE_STAIRS;
            	else if (((IndoorVertex)edge.getFromVertex()).getLevel() < ((IndoorVertex)edge.getToVertex()).getLevel())
            		step.relativeDirection = RelativeDirection.GO_UP_THE_STAIRS;
            	else step.relativeDirection = RelativeDirection.GO_THROUGH_THE_STAIRS;
            	
            	step.isDoor = (edge.getToVertex() instanceof IndoorDoorVertex);
            	
            	steps.add(step);
            	continue;
            }
            
            else if (edge instanceof IndoorElevatorEdge) {
            	step = createWalkStep(graph, forwardState);
            	createdNewStep = true;
            	step.streetName = (((IndoorElevatorEdge)edge).getName() != null) ? ((IndoorElevatorEdge)edge).getName() : "Elevator";
            	if(((IndoorVertex)edge.getFromVertex()).getLevel() > ((IndoorVertex)edge.getToVertex()).getLevel())
            		step.relativeDirection = RelativeDirection.GO_DOWN_WITH_THE_ELEVATOR;
            	else if (((IndoorVertex)edge.getFromVertex()).getLevel() < ((IndoorVertex)edge.getToVertex()).getLevel())
            		step.relativeDirection = RelativeDirection.GO_UP_WITH_THE_ELEVATOR;
            	else step.relativeDirection = RelativeDirection.ELEVATOR;
            	...
            	step = createWalkStep(graph, forwardState);
            	createdNewStep = true;
            	step.streetName = (((IndoorEdge)edge).getName() != null) ? ((IndoorEdge)edge).getName() : "Hallway";
            	step.relativeDirection = RelativeDirection.CONTINUE_THROUGH_THE_CORRIDOR;
            ...
            if (step == null) {
                ...
                if (edge.isRoundabout() || direction == RelativeDirection.CONTINUE) {
                    ...
                    if (edge instanceof StreetEdge || edge instanceof IndoorEdge) {
    ...
    private static WalkStep createWalkStep(Graph graph, State s) {
        ...
        step.level = getLevel(en.getFromVertex());
    ...
    private static double getLevel(Vertex vertex) {
		if(vertex instanceof IndoorVertex) return ((IndoorVertex)vertex).getLevel();
		else return 0.0;
	}
