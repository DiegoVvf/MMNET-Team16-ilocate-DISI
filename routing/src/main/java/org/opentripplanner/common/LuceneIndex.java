...
public class LuceneIndex {
    ...
    private void index() {
        try {
            ...
            // TODO: Use index for indoor
            for (IndoorVertex iv : Iterables.filter(graphIndex.vertexForId.values(), IndoorVertex.class)) {
                addRoom(writer, iv);
            }
            writer.close();
    ...
    private void addRoom(IndexWriter iwriter, IndoorVertex iv) throws IOException {
        if(iv.getName() != null && !iv.getName().equals("")) {
            // Add this name to the index
            Document doc = new Document();
            doc.add(new TextField("name", iv.getName(), Field.Store.YES));
            doc.add(new DoubleField("lat", iv.getLat(), Field.Store.YES));
            doc.add(new DoubleField("lon", iv.getLon(), Field.Store.YES));
            doc.add(new DoubleField("level", iv.getLevel(), Field.Store.YES));
            doc.add(new StringField("category", Category.ROOM.name(), Field.Store.YES));
            iwriter.addDocument(doc);
        }
    }
     ...
     * @param rooms Search for indoor rooms by their name
    ...
    public List<LuceneResult> query (String queryString, boolean autocomplete,
                                     boolean stops, boolean clusters, boolean corners, boolean rooms) {
        ...
        if (stops || clusters || corners || rooms) {
            ...
            if (rooms) {
                typeQuery.add(new TermQuery(new Term("category", Category.ROOM.name())), BooleanClause.Occur.SHOULD);
            }            
        ...
        try {
            ...
            for (int i = 0; i < docs.length; i++) {
                ...
                if (doc.getField("level") != null) {
                    lr.level = doc.getField("level").numericValue().doubleValue();
                }
                else {
                    lr.level = null;
                }
    ...
    public static enum Category { STOP, CORNER, CLUSTER, ROOM; }
}
